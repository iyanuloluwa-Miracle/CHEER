...
import { toPublicTipDto, type PublicTipDto } from './tips.types';
  ...
  async createTip(
    ...
    if (!amountResult.ok) {
      throw new BadRequestException({
        statusCode: 400,
        error: amountResult.reason,
        message: amountValidationMessage(amountResult.reason),
      });
    }
    ...
    try {
      ...
      if (err instanceof BachsProviderError) {
        throw new ServiceUnavailableException({
          ...
          message: bachsPublicMessage(err.kind),
        });
      }
      this.logger.error(
  ...
  private resolveCurrency(creatorCurrency: string, requested?: string): string {
    ...
    if (!(ALLOWED_CURRENCIES as readonly string[]).includes(authoritative)) {
      throw new BadRequestException({
        ...
        message: 'Creator currency is not supported for tips.',
      });
    }
    ...
    if (requested && requested.toUpperCase() !== authoritative) {
      throw new BadRequestException({
        ...
        message: 'Currency does not match this creator’s preferred currency.',
      });
    }
