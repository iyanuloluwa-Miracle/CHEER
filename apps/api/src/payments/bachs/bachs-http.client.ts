...
export class BachsHttpClient {
  private readonly logger = new Logger(BachsHttpClient.name);

  constructor(private readonly config: ConfigService) {}

  get isConfigured(): boolean {
    return Boolean(this.config.get<string>('BACHS_API_KEY')?.trim());
  }

  private baseUrl(): string {
    return (
      this.config.get<string>('BACHS_API_BASE_URL')?.replace(/\/$/, '') ||
      BACHS_SANDBOX_BASE_URL
    );
  }

  private apiKey(): string {
    const key = this.config.get<string>('BACHS_API_KEY')?.trim();
    if (!key) {
