...
export class BachsPaymentProvider implements PaymentProviderPort {
  ...
  private readonly logger = new Logger(BachsPaymentProvider.name);

  constructor(
    private readonly http: BachsHttpClient,
  ...
  async initializePayment(
    ...
    if (!session.checkout_id || !session.checkout_url) {
      this.logger.error('Bachs checkout response missing checkout_id/url');
      throw new BachsProviderError(
        'PROVIDER',
  ...
  handleWebhook(input: HandleWebhookInput): Promise<HandleWebhookResult> {
    const secret = this.config.get<string>('BACHS_WEBHOOK_SECRET')?.trim();
    if (!secret) {
      this.logger.error('BACHS_WEBHOOK_SECRET not configured');
    ...
    if (!valid) {
      this.logger.warn('Bachs webhook signature verification failed');
      return Promise.resolve({ acknowledged: false });
    }
    ...
    try {
      ...
      this.logger.warn('Bachs webhook body is not valid JSON');
      return Promise.resolve({ acknowledged: false });
    }
