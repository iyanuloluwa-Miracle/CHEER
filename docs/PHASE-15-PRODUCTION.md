...
| MongoDB | Managed (recommended) | Mongoose |
...
3. MongoDB via `MONGODB_URI` (no SQL migrate step)  
...
2. Provision HTTPS domain and managed MongoDB (Atlas).  
...
6. Set production `MONGODB_URI` and seed if needed.  
