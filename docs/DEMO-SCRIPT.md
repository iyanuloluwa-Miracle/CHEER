...
2. Optional: `CENCORI_API_KEY` for live AI; without it, local fallback copy still demos.
...
| 0:15–0:30 | Onboarding bio → **Polish with AI** | “Cencori polishes the bio in one tap — product-shaped AI, not a chatbot.” |
...
> “TippyMe owns the creator experience. Bachs moves the money. SendByte notifies. Cencori writes the human bits. One link for African builders.”

## If something fails live

- **No Bachs key:** Connect button still links a stub `acct_stub_…` and enables Friday payout messaging; tip uses stub checkout.
- **No Cencori key:** Bio polish + thank-you use deterministic local fallback — still demable.
